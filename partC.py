...
#Grouping amount of BTC by year
 smallAmountsbtc = smaller_joined_transactions.map(lambda x:(Helper_Methods.group_transactions(x[1][0][0]),float(x[1][1][1]))).reduceByKey(lambda x,y:x+y)
 largeAmountsbtc = larger_joined_transactions.map(lambda x:(Helper_Methods.group_transactions(x[1][0][0]),float(x[1][1][1]))).reduceByKey(lambda x,y:x+y)


#Grouping number of transactions by year
 smallAmountstra = smaller_joined_transactions.map(lambda x:(Helper_Methods.group_transactions(x[1][0][0]),1)).reduceByKey(lambda x,y:x+y)
 largeAmountstra = larger_joined_transactions.map(lambda x:(Helper_Methods.group_transactions(x[1][0][0]),1)).reduceByKey(lambda x,y:x+y)
